...
//该单片机是11.0592mhz = 11059200hz
//一个机器周期是12个hz,所以该单片机晶振在1s内能够产生921600个机器周期
//该项目的定时器技术方式是2的16位,2的16次方是65536
//即该定时器模式下最多记录65536个机器周期就会溢出中断
//可以根据这些数据来计算你所需要的时间是多少个机器周期并制定相应的数据
